...
class MysqlService {
    ...
    constructor(writeConnectionConfig, readConnectionConfig, logger) {
        ...
        this._readConnectionConfig = readConnectionConfig;
        this._writeConnectionConfig = writeConnectionConfig;

        this._createConnectionPools();

        this._logger = logger; // @TODO: throw an error if logger is not of correct type
        this._chance = new Chance();
    }

    _createConnectionPools() {
        this._writePool = Promise.promisifyAll(Mysql.createPool(this._writeConnectionConfig));

        if (this._readConnectionConfig) {
            const readConnectionConfigs = this._expandMysqlPoolConnectionConfig(this._readConnectionConfig);
    ...
    /**
     * Closes the existing pools and recreates them.
     * This can be useful if the existing pools encounter an acceptable failure that breaks the connection,
     * such as during a long-running script with retry logic.
     * @param {int} [delayMs] - Optional delay in milliseconds after closing the pools and recreating them.
     * @return {Promise} - Returns after delayMs with the new connection pools.
     */
    resetPools(delayMs) {

        delayMs = delayMs || 0;

        // End the pools asynchronously to ensure this function will return after delayMs.
        // Any lingering connections will have to be dealt-with by the implementing code.
        this.endPools();
