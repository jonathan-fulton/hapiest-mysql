...
class MysqlService {
    ...
    /**
     * @name MysqlServiceOptions
     * @type {Object}
     * @property {string} [dbTimezone] - should be a timezone in the moment-timezone list; if null/undefined, then system timezone will be used
     */
     ...
     */
    constructor(writeConnectionConfig, readConnectionConfig, logger, options) {
        ...
        this._validateDbTimezone();
    }

    _validateDbTimezone() {
        if (this._options.dbTimezone) {
            let isValidTimezone = false;
            const tzNames = MomentTimezone.tz.names();
            for (let i=0; i<tzNames.length; i++) {
                if (this._options.dbTimezone === tzNames[i]) {
                    isValidTimezone = true;
                    break;
                }
            }
            if (!isValidTimezone) {
                const err = new Error(`MysqlService: invalid database timezone value ${this._options.dbTimezone}`);
        ...
        return Promise.resolve()
            ...
            .then((rows) => {
                if (rows.length === 0) {
                    ...
                    const jsObj = JSON.parse(JSON.stringify(data), (key, value) => { // Convert dates back to Date objects
                        if (typeof value === 'string') {
                            const match = dateRegEx.exec(value);
                            if (match) {
                                return this._getDateFromDateString(value);
                            }
                        }
                        return value;
    ...
    _getDateFromDateString(dateString) {
        if (!this._options.dbTimezone) {
            return new Date(dateString);
        } else {
            return MomentTimezone.tz(dateString, this._options.dbTimezone).toDate();
        }
    }
    ...
    /** @returns {Promise} */
    endPools() {
        return this._writePool.endAsync()
            .then(() => Promise.each(this._readPools, pool => pool.endAsync()));
    }
     ...
     */
    ping() {
        const pools = this._readPools.concat(this._writePool);
        return Promise.each(pools, pool => pingPoolConnection(pool, this._logger));
