...
class MysqlDao {
    ...
    */
    ...
    /**
     * Similar to leftJoin(), but will operate in batches limited by maxBatchSize.
     ...
     */
    batchLeftJoin(objectsToJoin, lookupKey, maxBatchSize = 100, options) {
        ...
        const objectBatches = _.chunk(objectsToJoin, maxBatchSize);

        // Process each batch, then recombine.
        return Promise.map(objectBatches, objectBatch =>
            this.leftJoin(objectBatch, lookupKey, options)
        )
            .then(_.flatten)
            .then(objectsWithJoins => {
                return objectsWithJoins.length > 1 ? objectsWithJoins : _.head(objectsWithJoins);
            });

    }

    /**
     * Takes an array of VOs or object "source entities" and joins entries from this Dao onto each of them. It will take a property from the source entity
     * (defaulting to "id") and query this Dao for all rows matching that array of values (`SELECT * FROM table_name WHERE id IN (x, y, z, ...)`), and then join
     ...
     */
    leftJoin(objectsToJoin, lookupKey, options) {
        ...
        return this.getAll({[lookupKey]: joinValues})
            .then(vos => {
                ...
                return Promise.map(objectsToJoin, object => {
                    ...
                    // Add the target entities for the join under `resultsKey`.
