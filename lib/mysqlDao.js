...
class MysqlDao {
    ...
    */
    ...
    /**
     * Takes an array of VOs or objects "source entities" and joins entries from this Dao onto each of them. It will take a property from the source entity
     * (usually "id") and query this Dao for all rows matching that array of values (`SELECT * FROM table_name WHERE id IN (x, y, z, ...)`), and then join
     * the matching target entities for each source entity and add them under a new property on the source entity (defaulting to the table name for this Dao).
     * @param {VO[]|VO|Object[]|Object} sourceEntities - Source entity or entities which should be joined-onto.
     * @param {string} targetJoinKey - The target entity key to use for the join.
     * @param {string} [sourceJoinKey = "id"] - The source entity key to use for the join.
     * @param {string} [joinedEntitiesKey] - The source entity key onto which the joined entities will be added. Defaults to the value of `this.tableName`.
     * @param {int} [maxBatchSize = 100] - The batch size for querying this Dao's table (will attempt to select all entries for the
     */
    batchJoin(sourceEntities, targetJoinKey, sourceJoinKey = 'id', joinedEntitiesKey, maxBatchSize = 100) {

        sourceEntities = _.castArray(sourceEntities);

        const sourceJoinValues = sourceEntities.map(sourceEntity => sourceEntity[sourceJoinKey]);

        return this.getAll({[targetJoinKey]: sourceJoinValues})
            .then(targetEntities => {
                // For each source entity, find all matching target entities to join.
                return Promise.map(sourceEntities, sourceEntity => {
                    // Pull all target entities which match source entities. NOTE: This mutates `targetEntities` (reducing the set each time).
                    const targetEntitiesToJoin = _.remove(targetEntities, targetEntity => sourceEntity[sourceJoinKey] === targetEntity[targetJoinKey]);
                    // Add the target entities for the join under `joinedEntitiesKey`.
                    sourceEntity[joinedEntitiesKey] = targetEntitiesToJoin;
                    return sourceEntity;
                });
            });
    }
